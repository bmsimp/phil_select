...
class PhilmontScorer:
    ...
    def calculate_itinerary_scores(self, method='Total'):
        ...
        for itin in itineraries:
            ...
            score_components = {
                'program_score': self._calculate_program_score(itin_dict['id'], program_scores, conn),
                'difficulty_score': self._calculate_difficulty_score(itin_dict, crew_prefs),
    ...
    def _get_crew_preferences(self):
        ...
        conn = sqlite3.connect('philmont_selection.db')
        cursor = conn.execute('SELECT * FROM crew_preferences WHERE crew_id = ?', (self.crew_id,))
...
def preferences():
    ...
    crew_members = CrewMember.query.filter_by(crew_id=crew_id).order_by(CrewMember.member_number).all()
...
def save_preferences():
    ...
    preferences.area_rank_central = int(request.form.get('area_rank_central', 0)) or None
    preferences.area_rank_north = int(request.form.get('area_rank_north', 0)) or None
    preferences.area_rank_valle_vidal = int(request.form.get('area_rank_valle_vidal', 0)) or None
    
    preferences.max_altitude_important = 'max_altitude_important' in request.form
    preferences.max_altitude_threshold = int(request.form.get('max_altitude_threshold', 0)) or None
    ...
    preferences.difficulty_super_strenuous = 'difficulty_super_strenuous' in request.form
